...
int menorTempo(int qtd_est, int qtd_trans, int pos, int pulo, int *tarefa1, int *tarefa2, int *trans1, int *trans2, int *inicio, int *final, int menor){
    ...
    while(i < qtd_est){
        if(i < qtd_trans){
            if(linha == 1){
                tempo1 += tarefa1[i];
                tempo1 += trans1[pos_aux];
                pos_aux+=pulo;
                linha = 2;
            }
            else{
                tempo1 += tarefa2[i];
                tempo1 += trans2[pos_aux];
                pos_aux+=pulo;
                linha = 1;
            }
        }
    ...
    // tempo na linha 2
    linha = 2; i=0; pos_aux=pos;
    while(i < qtd_est){
        if(i < qtd_trans) {
            if(linha == 1){
                tempo2 += tarefa1[i];
                tempo2 += trans1[pos_aux];
                pos_aux+=pulo;
                linha = 2;
            }
            else{
                tempo2 += tarefa2[i];
                tempo2 += trans2[pos_aux];
                pos_aux+=pulo;
                linha = 1;
            }
        }
...
int main(){
    ...
    while(n > 0){
        ...
        cout << endl << "Menor tempo = " << menorTempo(qtd_est, qtd_est-1, 0, 1, tarefa1, tarefa2, trans1, trans2, inicio, final, 10000) << endl;
